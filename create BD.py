...
class Income(db.Entity):
    ...
    products = Set(Product, column='incomes')
...
class Buy(db.Entity):
    ...
    products = Set(Product, column='buys')
    list_buys = Optional('List_buy', column='buys')


class List_buy(db.Entity):
    id = PrimaryKey(int, auto=True)
    buys = Set(Buy, cascade_delete=False)  # NotImplementedError:', column='list_buys' Parameter 'column' is not allowed for many-to-one attribute List_buy.buys
...
with db_session:
    #p = Product(Partnumber='Partnumber1', Measure ='Measure1', Name='Name1')  # Name=hashMap.get('name_product'),
     for number in range(10):
        i = Income(qty_income=10-number)#qty_income=0, products=p
        List_income[2].incomes.add(i)
